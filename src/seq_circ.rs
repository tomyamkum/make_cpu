...
impl Reg {
	...
	pub fn load(&mut self, input: &Vec<bool>, load: bool) -> Vec<bool> {
		...
		let mut ans = Vec::new();
		ans.push(self.bit[0].dff.state);
		ans.push(self.bit[1].dff.state);
		ans.push(self.bit[2].dff.state);
		ans.push(self.bit[3].dff.state);
		ans.push(self.bit[4].dff.state);
		ans.push(self.bit[5].dff.state);
		ans.push(self.bit[6].dff.state);
		ans.push(self.bit[7].dff.state);
		ans.push(self.bit[8].dff.state);
		ans.push(self.bit[9].dff.state);
		ans.push(self.bit[10].dff.state);
		ans.push(self.bit[11].dff.state);
		ans.push(self.bit[12].dff.state);
		ans.push(self.bit[13].dff.state);
		ans.push(self.bit[14].dff.state);
		ans.push(self.bit[15].dff.state);
		ans
	}
}
...
impl Ram8 {
	pub fn new(initial_state: &Vec<Vec<bool>>) -> Ram8 {
		Ram8 {
			reg: vec![Reg::new(&initial_state[0]), Reg::new(&initial_state[1]), Reg::new(&initial_state[2]), Reg::new(&initial_state[3]), Reg::new(&initial_state[4]), Reg::new(&initial_state[5]), Reg::new(&initial_state[6]), Reg::new(&initial_state[7])],
		}
	}
	pub fn load(&mut self, input: &Vec<bool>, address: usize, load: bool) -> Vec< Vec<bool> > {
		let tmpinput = vec![true; 16];
		self.reg[address] = Reg::new(&mux16(&self.reg[address].load(&tmpinput, false), &input, load));
		
		let mut ans = Vec::new();
		ans.push(self.reg[0].load(&tmpinput, false));
		ans.push(self.reg[1].load(&tmpinput, false));
		ans.push(self.reg[2].load(&tmpinput, false));
		ans.push(self.reg[3].load(&tmpinput, false));
		ans.push(self.reg[4].load(&tmpinput, false));
		ans.push(self.reg[5].load(&tmpinput, false));
		ans.push(self.reg[6].load(&tmpinput, false));
		ans.push(self.reg[7].load(&tmpinput, false));
		ans
	}
}
